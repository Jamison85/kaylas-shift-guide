...
const reactionMoods=["point","thinking","celebrate","ready","tired"];

export default function App(){
 const day=dateKey(),[progress,setProgress]=useLocalStorage(`kayla-guide-${day}`,{completed:[],currentIndex:0});
 const [screen,setScreen]=useState("splash"),[motion,setMotion]=useState(0),[showReaction,setShowReaction]=useState(false);
 ...
 useEffect(()=>{if(!showReaction)return;const id=setTimeout(()=>setShowReaction(false),900);return()=>clearTimeout(id)},[showReaction,motion]);

 const react=()=>{setMotion(x=>x+1);setShowReaction(true)};
 const go=i=>{const n=Math.max(0,Math.min(i,guideTasks.length-1));setProgress(p=>({...p,currentIndex:n}));react();setScreen("task")};
 const complete=()=>{const was=done.has(task.id),next=was?progress.completed.filter(x=>x!==task.id):[...(progress.completed||[]),task.id];setProgress(p=>({...p,completed:next,currentIndex:!was&&index<guideTasks.length-1?index+1:index}));react();if(!was&&count+1===guideTasks.length)setTimeout(()=>setScreen("complete"),300)};
 const reset=()=>{setProgress({completed:[],currentIndex:0});setScreen("home");react()};
